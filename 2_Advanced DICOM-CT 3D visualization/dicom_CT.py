...
#
def vtk_render_gif(renderer, N, name, Roll = False, Azimuth = False, Elevation = False, Actor = None,
                   RotateX = False, RotateY = False, RotateZ = False, Zoom = 0, Dolly = 0, standard = True,
...
def CreateTissue(reader, ThrIn, ThrOut, color = "skeleton", isoValue = 127.5):
    selectTissue = vtk.vtkImageThreshold() # Threshold
    selectTissue.ThresholdBetween(ThrIn,ThrOut)
    selectTissue.ReplaceInOn()
    selectTissue.SetInValue(255) # 255
...
def render_lungs(workdir, PathDicom, patient):
    reader = vtk.vtkDICOMImageReader()
    reader.SetDirectoryName(PathDicom) # 读入文件
    reader.Update()
        
    # disp = Display().start()
    renderer = vtk.vtkRenderer()

    renderer.AddActor(CreateTissue(reader, -110, -100,"lung"))
    ...
    name = workdir + patient  # output_p1_lungs
...
def main():
    ...
    render_lungs(workdir,PathDicom,patient="lung_1_1645")
